...
import static mediator.Preconditions.checkArgument;
...
public class Pipelinr implements Pipeline {
    ...
    /**
     * This constructor is deprecated since v0.5. Use empty constructor and provide extra dependencies using with* methods.
     */
    @Deprecated
    ...
    public Pipelinr with(Supplier<NotificationHandlingStrategy> notificationHandlingStrategySupplier) {
        checkArgument(notificationHandlingStrategySupplier, "Notification handling strategy supplier must not be null");
    ...
    public <R, C extends Command<R>> R send(C command) {
        ...
        if (legacyPipelineSteps) {
            PipelineStep.Next<R> handleCommand = new Handle<R, C>(command);
            return steps
                    .supplyEx()
                    .foldRight(handleCommand, (step, next) -> () -> step.invoke(command, next))
                    .invoke();
        } else {
            Command.Middleware.Next<R> handleCommand = new HandleCommand<R, C>(command);
    ...
    public <N extends Notification> void send(N notification) {
        ...
        List<Runnable> runnableNotifications = notificationHandlers
                .supply()
                .filter(it -> it.matches(notification))
                .map(it -> {
                    Notification.Middleware.Next handleNotification = () -> it.handle(notification);
                    return (Runnable) () -> notificationMiddlewares
                            .supplyEx()
                            .foldRight(handleNotification, (step, next) -> () -> step.invoke(notification, next))
                            .invoke();
                })
                .collect(toList());

        NotificationHandlingStrategy notificationHandlingStrategy = notificationHandlingStrategySupplier.get();
